...
export const fetchQuotes = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
        try {
            ...
            const response = await GetQuotesApiService.getInstance().getQuotes({
...
export const createQuote = createAsyncThunk(
    ...
    async (quoteData: QuotesState["form"], { rejectWithValue }) => {
        try {
            ...
            const response = await GetQuotesApiService.getInstance().createQuote(quoteData, {
                headers: { authorization: `Bearer ${token}` },
            });
            return await response;
        } catch (error: any) {
...
export const getQuoteDetails = createAsyncThunk(
    ...
    async (quoteId: string, { rejectWithValue }) => {
        try {
            const token = localStorage.getItem("token");
            const response = await GetQuotesApiService.getInstance().getQuoteById(quoteId, {
                headers: { authorization: `Bearer ${token}` },
            });
            return await response;
        } catch (error: any) {
